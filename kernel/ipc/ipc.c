...
void init_ipc()
{
	...
	memzero(ipc_ports, KMEM_IPC_EXCL_SIZE);
	...
	//Mark buffer pages used by header stuff as used
	int used_pages = ((uint32_t)ipc_buffer_start-KMEM_IPC_EXCL_LOC) / PAGE_SIZE;
	ipc_buffer_pages = KMEM_IPC_EXCL_SIZE / PAGE_SIZE;
	for(int x=0; x<ipc_buffer_pages; x++)
	{
		if(x<used_pages)
			ipc_res_buffer_map[x] = -1;
		void* addr = (void*)KMEM_IPC_EXCL_LOC + x * PAGE_SIZE;
		kernel_map_page(addr, addr, KERNEL_PAGE_FLAGS|PG_User);
	}
	
	/*
	terminal_writestring("IPC Info\n");
	terminal_writeuint32(ipc_ports);
	terminal_writeuint32(ipc_buffer_tbl);
	terminal_writeuint32(ipc_res_buffer_map);
	terminal_writeuint32(ipc_buffer_start);
	terminal_writeuint32(ipc_buffer_pages);
	...
	for(int x=0; x<ipc_buffer_pages; x++)
	{
		if(ipc_res_buffer_map[x] == 0)
		{
			...
			if(found_pgs==needed_pgs)
			{
				...
				return ((void*)KMEM_IPC_EXCL_LOC+(x-needed_pgs+1)*PAGE_SIZE);
			}
		}
		else
			found_pgs = 0;
